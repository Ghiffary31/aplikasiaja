...
if(isset($_POST['insert'])){
	...
	if($query){
		...
		if($fdata['service']){
			...
			$sql = "INSERT INTO service (id_kendaraan, berlaku_service, biaya_service, tgl_input, tgl_pelaksanaan, id_user, keterangan, status, id_penggunakendaraan) 
					VALUES ('{$id_kendaraan}', '{$tgl_berlaku}', '{$biaya}', '{$today}', '{$tgl_pelaksanaan}', '{$_SESSION['id_user']}', '{$service['keterangan']}', 'active', '{$nama_pengguna}')";
			$query = query($sql);
		}
